...
if [[ -n "$1" ]]; then
  ...
  # 오픈 스택 명령어를 이용한 인스턴스 전원 종료 명령 생성
  openstack server list --host $Shost --all-projects -c ID -f value | awk '{print "openstack server stop "$1}' > /home/stack/vm_poweroff_$1.sh
else
