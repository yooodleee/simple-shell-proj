...
if [[ -n $1 ]]; then
  ...
  # 정규 표현식과 다르다면 메시지를 출력하고 script 종료
...
# chrony가 설치되어 있는지, ntp가 설치되어 있는지 환경 설정 파일을 통해 확인
...
# 파라미터로 입력 받은 IP가 있고, chrony이면 allow 설정
if [[ -n $1 && -f /etc/chrony.conf ]]; then
  sed -i "/^#allow/ a \allow $1" $conf
# 파라미터로 입력 받은 IP가 있고, ntp이면 restrict 설정
elif [[ -n $1 && -f /etc/ntp.conf ]]; then
  transfer_iprange $1
  restrict="restrict $ip mask $range nomodify notrap"
